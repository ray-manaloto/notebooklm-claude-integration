...
if [[ -n "${NOTEBOOK_IDS}" ]]; then
  ...
  codex --enable skills exec "Use the notebooklm-patterns skill. List all notebooks, then ask each notebook (via notebook_id): '${QUESTION}'. Aggregate responses labeled by notebook name and include citations. If any ask_question times out, retry once with browser_options timeout_ms=60000. If any response is off-topic, retry once with a narrower prompt that starts with: 'Answer ONLY about: ${QUESTION}'. If it still drifts, report a likely notebook-content mismatch."
fi
